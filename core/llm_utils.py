...
class DocumentCreation:
    ...
    def generate_full_workflow(self, theme: str, manual_files: List[str] = None,
        ...
        if use_auto_selection and self.retriever:
            print(f"\n🔍 步骤3: 自动选择参考文档...")
            reference_docs = self.auto_select_reference_docs(abstract, top_k_docs)
            print("✅ 文档选择完成")
        ...
        # 步骤4: 生成最终文章
        print(f"\n✍️  步骤4: 生成最终文章...")
        article = self.article_generation.generate_article(theme, abstract, reference_docs)
        print(article)
        print("✅ 文章生成完成")
    ...
    def save_results(self, results: Dict[str, str], output_dir: str = "output"):
        ...
        # 保存分析
        analysis_file = os.path.join(output_dir, f"{timestamp}_{theme_safe}_分析.txt")
        with open(analysis_file, 'w', encoding='utf-8') as f:
            f.write(f"主题: {results['theme']}\n")
            f.write("=" * 60 + "\n\n")
            f.write(results['analysis'])
        files_saved.append(analysis_file)

        # 保存摘要
        abstract_file = os.path.join(output_dir, f"{timestamp}_{theme_safe}_摘要.txt")
        ...
        # 保存文章
        article_file = os.path.join(output_dir, f"{timestamp}_{theme_safe}_文章.md")
        ...
        if results['reference_docs']:
            docs_file = os.path.join(output_dir, f"{timestamp}_{theme_safe}_参考文档.txt")
