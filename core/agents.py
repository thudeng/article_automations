...
def abstract(theme,api_key,files_path:List[str],analysis):
    client = OpenAI(api_key=api_key, base_url="https://api.deepseek.com")
    content_dict={}
    for file_path in files_path:
        with open("..\config\strategies\strategies_base\\"+file_path,"r",encoding="utf-8") as file:
    ...
    response = client.chat.completions.create(
                        ...
                        messages=[
                            ...
                            4.针对这个主题，你的摘要应容易通过BM25算法检索到相关类型的文章
...
def Analysis(theme,api_key):
    ...
    response = client.chat.completions.create(
                        model="deepseek-chat",
                        messages=[
                            {"role": "system", "content": """你是一位精通亚马逊SP广告运营的专家，DeepBI是一款基于AI智能的广告投放助手，你将分析结合DeepBI的策略，给出亚马逊广告运营痛点的破解之道"""},
                            {"role": "user", "content": f"""
                            #痛点主题
                            {theme}
                            #DeepBI相关策略的理解
...
def Article_Creation(theme,api_key,files_path,Abstract):
    client = OpenAI(api_key=api_key, base_url="https://api.deepseek.com")
    content_dict = {}
    for file_path in files_path:
        with open("..\config\strategies\\" + file_path, "r", encoding="utf-8") as file:
            content_dict[file_path.strip(".md").strip(".txt")] = file.read()
    with open("..\\templates\Marketing_templates.md","r",encoding="utf-8") as template:
        templates = template.read()
    response = client.chat.completions.create(
model="deepseek-chat",
messages=[{"role": "system", "content": """你是一位亚马逊SP广告运营的商家也是一名知乎、头条、CSDN等图文平台的优秀创作者。DeepBI是一款基于AI智能的广告投放助手，你所在公司使用了DeepBI产品后实现了"由亏转盈"的"破局"和"改命"。现在你怀揣着感恩之心，去写一篇推介DeepBI产品的文章"""},
{"role": "user", "content": f"""
#主题
{theme}
#目的
